1.generate package.json
    npm init -y
    after running it change the description and main to your appropriate liking(main is the entry point)
...
----
mongodb is a document oriented database belonging to no sql data base
each collection is compared to table
each document is compared to a record
document has field value pairs
hierarchy is database->collection->documents
camelcasing in db commands
what commands i used tody to tell teddy:
use mydb1
show databases/show dbs-shows ponly non empty databases

//creating collections 
db.createCollection("users")   db is the current database,users is the table name
db.collection-name.insertOne()   for every document mongodb no concept of primary key,it will automatically attaches unique value to it
select the data and press up arrow button
db.collection-name.insertMany()-array of documents
findOne(condition)-returns only one document
    .find({field:{query-op:value}})   value is the object
    .find({field:{query-op:[cond1,cond2,.......]}}) 
find(condition)-returns all the document which satisfies the condition
    both can be called with or without condition
when we use the above 2 commands it returns a cursor,just like a pointer to display data

$in-matches any of the values that exist in an array
1.find the users whose age is 21,23 or 27
2.find the users whose age is above 25 and city hyd

updateOne()
updateMany()//update many is used to update many documents of the same table
3.add new skill angular
...
1.find emp with React Skills-done
2.add new skills "Express"-done
3.add new Experience-
4.delete first exprerience
5.add address to emp whose name is vikas